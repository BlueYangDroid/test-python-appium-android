...
class AppiumServer:
    ...
    def start_server(self):
        ...
        for i in range(0, len(self.kwargs)):
            cmd = "appium --session-override  -p %s -bp %s -U %s" % (
                self.kwargs[i]["port"], self.kwargs[i]["bport"], self.kwargs[i]["device"])
            print(cmd)
    ...
    def simple_start_server(self):
        """start the appium server
        """
        cmd = "appium"
        # 生成appium的log
        # cmd = 'appium --address 127.0.0.1 --port 4723 --log "appium.log" --log-timestamp --local-timezone --session-override'
        appium = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        while True:
            appium_line = appium.stdout.readline().strip().decode()
            time.sleep(1)
            print("---------simple_start_server----------")
            print("appium_line [" + appium_line + "]")
            if 'listener started' in appium_line or 'Welcome' in appium_line:
                print("----server_ 成功---")
                break
    ...
    def simple_stop_server(self):
        sysstr = platform.system()
        print("---- simple_stop_server ---- " + sysstr)
        os.popen("taskkill /f /im node.exe")


    def stop_server(self, devices):
        sysstr = platform.system()
        print("---- stop_server ---- " + sysstr)
        ...
        if sysstr == 'Windows':
            ...
            for device in devices:
                # mac
                cmd = "lsof -i :{0}".format(device["port"])     #4723
